...
List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("Coy K. Adams", DateTime.Parse("March 1, 1976"), "456-892-1307", "CoyKAdams@dayrep.com", "2411 Blue Spruce Lane Baltimore, MD 21202", 3784),
...
////Task1 (rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag)

//List<Debtor> debt = debtors.Where(d => d.Email.EndsWith("dayrep.com")|| d.Email.EndsWith("rhyta.com")).ToList();

//debt.ForEach(d => { Console.WriteLine(d);
//    Console.WriteLine();
//});
...
////Task2 (Yashi 26-dan 36-ya qeder olan borclulari cixartmag)

//List<Debtor> debt = debtors.Where(d => d.BirthDay.Year>=1988 &&  d.BirthDay.Year <= 1998).ToList();
// debt.ForEach(d => {
//Console.WriteLine(d);
//     Console.WriteLine();
// });
...
////Task3 (Borcu 5000-den cox olmayan borclularic cixartmag)

//List<Debtor> debt = debtors.Where(d => d.Debt<=5000).ToList();
//debt.ForEach(d =>
//{
//    Console.WriteLine(d);
//    Console.WriteLine();
...
////Task4 (Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq)

//List<Debtor> debt = debtors
//    .Where(d => d.FullName.Length > 18)
//    .Where(d => d.Phone.Count(c => c == '7') > 1) .ToList(); 
//debt.ForEach(d =>
//{
//    Console.WriteLine(d);
//    Console.WriteLine();
...
////Task5 (Qishda anadan olan borclulari cixardmaq)

//List<Debtor> debt = debtors.Where(d => d.BirthDay.Month == 1 || d.BirthDay.Month == 2 || d.BirthDay.Month == 12).ToList();
//debt.ForEach(d =>
//{
//    Console.WriteLine(d);
//    Console.WriteLine();
...
//List<Debtor> debt = debtors.Where(d => d.Debt > debtors.Average(d => d.Debt)).ToList();
//debt.Sort((d1, d2) => d1.FullName.CompareTo(d2.FullName));
//debt.Sort((d1, d2) => d1.Debt.CompareTo(d2.Debt));
//debt.ForEach(d =>
//{
//    Console.WriteLine(d);
//    Console.WriteLine();
...
////Task7 (Telefon nomresinde 8 olmayan borclularin yalniz familyasin,yashin ve umumi borcun meblegin cixarmaq)


//List<Debtor> debt = debtors.Where(d=> !(d.Phone.Contains("8"))).ToList();
//debt.ForEach(d =>
//{
//    Console.Write("Name: "+d.FullName);
//    Console.Write("  Age: "+(DateTime.Now.Year- d.BirthDay.Year));
...
//List<Debtor> debt = debtors.Where(d => d.FullName.GroupBy(c => c).Any(g => g.Count() >= 3)).ToList();
//debt.Sort((Debtor d1, Debtor d2) => d1.FullName.CompareTo(d2.FullName));
//debt.ForEach(d =>
//{
//    Console.WriteLine(d);
//    Console.WriteLine();
...
////Task9 (borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq)


//var year =  debtors.GroupBy(d => d.BirthDay.Year).OrderByDescending(d=>d.Count()).First().Key;
...
////Task10 (Borcu en boyuk olan 5 borclunun siyahisini cixartmaq)


//debtors.Sort((Debtor d1, Debtor d2) => d2.Debt.CompareTo(d1.Debt));
//List<Debtor> debt = debtors.Take(5).ToList();
//debt.ForEach(d =>
//{
//    Console.WriteLine(d);
//    Console.WriteLine();
...
////Task 11 (Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq)
...
////Task 12 (2ci dunya muharibesin gormush borclularin siyahisi cixartmaq)


//List<Debtor> debt = debtors.Where(d => d.BirthDay.Year <=1945).ToList();
//debt.ForEach(d =>
//{
//    Console.WriteLine(d);
//    Console.WriteLine();
...
////Task 13 (Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq)



//List<Debtor> debt = debtors.Where(d => d.Phone.Replace("-", "").Distinct().Count() == d.Phone.Replace("-", "").Length).ToList();
//debt.ForEach(d =>
//{
//    Console.WriteLine(d);
//    Console.WriteLine();
...
//List<Debtor> debt = debtors.Where(d => d.Debt<=(500*(DateTime.Now.Month - d.BirthDay.Month))).ToList();
//debt.ForEach(d =>
//{
//    Console.WriteLine(d);
//    Console.WriteLine();
...
////Task 15 (Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq)


//List<Debtor> debt = debtors.Where(d => (d.FullName.Contains("s")&&d.FullName.Contains("m")&&d.FullName.Contains("i")&& d.FullName.Contains("l")&& d.FullName.Contains("e"))).ToList();
//debt.ForEach(d =>
//{
//    Console.WriteLine(d);
//    Console.WriteLine();
